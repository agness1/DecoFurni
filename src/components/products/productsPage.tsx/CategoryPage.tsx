      ...
      allDataProducts.map((productGroup: any, index) => (
        ...
        >
          {Object.keys(productGroup).map((productId) => {
            ...
            if ((product.category == path && product.feature == features) && (product.color == color || color == 'allColors') && (product.price > price[0] && product.price <= price[1])  ) {
              return (
                <ProductItem key={product.id} product={product}/>
              );
            } else if ((features == "default" && product.category == path) && (product.color == color || color == 'allColors') && (product.price > price[0] && product.price <= price[1]) )  {
              return (
                <ProductItem key={product.id} product={product}/>
              );
            }
